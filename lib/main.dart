...
//       home: const ProductQueryScreen(),
//
//       // Optional: Add route generation for navigation
//       onGenerateRoute: (settings) {
//         switch (settings.name) {
//           case '/':
//             return MaterialPageRoute(
//               builder: (context) => const ProductQueryScreen(),
//             );
//           default:
//             return MaterialPageRoute(
//               builder: (context) => const ProductQueryScreen(),
...
import 'package:flutter/material.dart';
import 'package:bitsdojo_window/bitsdojo_window.dart'; // Import the package
import 'screens/product_query_screen.dart';

void main() {
  runApp(const ProductQueryApp());

  // Add this to initialize the custom window
  doWhenWindowReady(() {
    final win = appWindow;
    const initialSize = Size(1200, 800); // Set an initial size
    win.minSize = const Size(800, 600);
    win.size = initialSize;
    win.alignment = Alignment.center;
    win.title = "Product Query App";
    win.show();
  });
}

// Main app widget with custom window frame
class ProductQueryApp extends StatelessWidget {
  const ProductQueryApp({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      themeMode: ThemeMode.system, // Respects system theme preference
      home: Scaffold(
        // Use a Scaffold to provide a body for the custom window elements
        body: Column(
          children: [
            // Custom Title Bar
            WindowTitleBarBox(
...
// Custom window buttons widget with only a close button
